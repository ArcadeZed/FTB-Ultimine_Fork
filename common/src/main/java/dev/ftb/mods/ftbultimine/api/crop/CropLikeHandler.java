...
/// Implementations of this can be registered via [RegisterCropLikeEvent] to add harvesting behaviour for custom
/// crop-like blocks.
public interface CropLikeHandler {
    ...
    /// Actually harvest the crop, adding any item drops to the [Consumer] object. This is called for each
    /// matching block in the current Ultimine shape, when [#isApplicable(Level, BlockPos, BlockState)] has
    ...
    /// @param itemCollector collect harvesting drops into this
    boolean doHarvesting(Player player, BlockPos pos, BlockState state, Consumer<ItemStack> itemCollector);

    /// Check if the two block states are equivalent for harvesting purposes, i.e. if `original` would be
    /// harvested, would `state` also be harvested?
    ///
