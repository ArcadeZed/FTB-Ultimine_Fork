...
/// Implementations of this can be registered via [RegisterBlockBreakHandlerEvent] to add custom block-breaking
...
public interface BlockBreakHandler {
    ...
    /// broken. You can check if this is the original block by comparing `pos` and `hitResult.getBlockPos()`.
    ///
    /// Your implementation should initially check that the block in question is one that you specifically wish to handle,
    /// and immediately return `PASS` if not. If the block is of interest, you should carry out your custom
    /// block-breaking logic and return `SUCCESS` or `FAIL` as appropriate.
    ///
    ...
    /// @return `PASS` if the candidate block is not of interest, `SUCCESS` if the candidate block was successfully broken,
    ///      or `FAIL` if the candidate block could not be broken
