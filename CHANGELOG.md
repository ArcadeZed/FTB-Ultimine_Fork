...
### Fixed
* Block interactions was being blocked on Fabric. Thanks to [#181](https://github.com/FTBTeam/FTB-Ultimine/pull/181)
...
### Fixed
* Fixed access widener issue on Fabric
...
### Fixed
* Fixed new single-crop-harvesting feature not working with fake players (automated clickers)

## [2101.1.8]

### Fixed
* Fixed issues with selection of flower-type blocks
  * Technically, any blocks which extend `BushBlock` but which are not crops (e.g. flowers)

## [2101.1.7]

### Added
* Added support for single-crop-harvesting, without requiring the Ultimine key to be held
  * Enabled by default; see `single_crop_harvesting` in `config/ftbultimine-server.snbt`
* Added API to allow other mods to register custom block selection handlers
  * Allows for extended control over what blocks can be included in the selection when the Ultimine key is held
  * E.g. could query some extended block entity data for the blocks

### Fixed
* Added missing translation in server config screen for crystal harvesting

## [2101.1.6]

### Fixed
* Hotfix: (Fabric only) fixed server crash when right-clicking to ultimine-harvest crops
