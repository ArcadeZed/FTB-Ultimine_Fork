...
/// Implementations of this can be registered via [RegisterBlockSelectionHandlerEvent] to add custom block-selection
...
public interface BlockSelectionHandler {
    /// Perform a custom check to see if a block should be included in the selection for a possible ultimining operation,
    /// based on its block position and blockstate. The original block position and blockstate are also available, for
    /// comparison purposes.
    ///
    ...
    Result customSelectionCheck(Player player, BlockPos origPos, BlockPos pos, BlockState origState, BlockState state);
