...
/// The context for an ultimining operation, passed to [Shape#getBlocks(ShapeContext)]. This provides all the
/// information needed for a shape to determine the blocks that should be affected by the operation. In particular,
/// implementations of [Shape] should call [#check(BlockPos)] to determine if a particular block position
...
public record ShapeContext(ServerPlayer player, BlockPos origPos, Direction face, BlockState origState, Matcher matcher, int maxBlocks) {
	...
	/// Called by implementations of [Shape] to check if a particular block position should be part of an
